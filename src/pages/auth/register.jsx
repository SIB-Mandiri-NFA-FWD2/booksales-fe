...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.name.trim() !== "" && !/^[A-Za-z\s]+$/.test(formData.name)) {
      meesageError.name = "Cannot contain numbers or special characters.";
    }

    if (formData.password && formData.password.length < 8) {
      meesageError.password = "Password must be at least 8 characters long.";
    }

    if (Object.keys(meesageError).length > 0) {
      setError(meesageError);
      return;
    }  
          ...
          <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
            <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              >
                ...
                >
                  {loading ? "Setting up your account..." : "Create an account"}
                </button>
